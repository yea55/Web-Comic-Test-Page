 ...
 * 
 * account_creation.php allows users to create artist accounts
...
echo "Or click <a href='index.php'>here to go back to the index</a>.<br />";
	...
	isset($_POST['conf_pwd']))
{
	...
	if ($new_pwd == $conf_pwd)
	{
		...
		$query = "SELECT * FROM uploaders WHERE username='" . $new_user . "'";
		...
		if (!$row)
		{
			...
			$query = "INSERT INTO uploaders VALUES" .
				"('$new_user', '$new_pwd', '', '')";
...
echo <<<_END
<form action ="account_creation.php" method="post"><pre>
